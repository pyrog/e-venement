...
class Seater
{
  ...
  {
    $this->kept = new Doctrine_Collection(Doctrine::getTable('Seat'));
    $this->done = new Doctrine_Collection(Doctrine::getTable('Seat'));
    $this->query = Doctrine::getTable('Seat')->createQuery('s')
      ->select('s.*, n.*')
      ->leftJoin('s.SeatedPlan sp')
      ->leftJoin('sp.Workspaces spw')
      ->leftJoin('spw.Gauge g')
      ->andWhere('g.id = ?', $this->gauge_id = $gauge_id)
  ...
  public function findOrphansWith($seats = NULL)
  {
    ...
    foreach ( $this->kept as $seat )
      $seats->merge($this->_findOrphansWithWalk($seat));
  ...
  protected function _findOrphansWithWalk(Seat $seat)
  {
    ...
    foreach ( $seat->Neighbors as $n )
    {
      ...
      if ( isset($this->kept[$n->id])                           // if it's one of the selected seats
      ...
      {
        if ( isset($this->kept[$n2->id])                           // if it's one of the selected seats
  ...
  {
    if ( !$exclude )
      $exclude = new Doctrine_Collection(Doctrine::getTable('Seat'));
    $this->done = $exclude;
