...
<?php foreach ( $byUser as $u ): ?>
  <tr class="<?php echo ($class = !$class) ? 'overlined' : '' ?>">
    ...
    <td class="average"><?php echo $u->nb_paying > 0 ? format_currency(($u->income+$u->outcome)/$u->nb_paying,'€') : 'N/A' ?></td>
    <td class="nb"><?php echo $u->nb_free; $total['nb_free'] += $u->nb_free ?></td>
    <td class="average"><?php echo $u->nb_free+$u->nb_paying > 0 ? format_currency(($u->income+$u->outcome)/($u->nb_free+$u->nb_paying),'€') : 'N/A' ?></td>
...
<tfoot>
  <tr class="<?php echo ($class = !$class) ? 'overlined' : '' ?>">
    ...
    <td class="average"><?php echo $total['nb_paying'] > 0 ? format_currency(($total['value+']+$total['value-'])/$total['nb_paying'],'€') : 'N/A' ?></td>
    <td class="nb"><?php echo $total['nb_free'] ?></td>
    <td class="average"><?php echo $total['nb_free']+$total['nb_paying'] > 0 ? format_currency(($total['value+']+$total['value-'])/($total['nb_free']+$total['nb_paying']),'€') : 'N/A' ?></td>
