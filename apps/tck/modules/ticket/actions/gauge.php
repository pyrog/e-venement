...
<?php
    ...
    $q->from('Manifestation m')
      ...
      ->addSelect('m.id')
      ->addSelect('sum(t.printed) AS sells')
      ->addSelect('sum(NOT t.printed AND t.transaction_id IN (SELECT o.transaction_id FROM order o)) AS orders')
      ->addSelect('sum(NOT t.printed AND t.transaction_id NOT IN (SELECT o2.transaction_id FROM order o2)) AS demands')
      ->andWhere('m.id = ?',$mid)
