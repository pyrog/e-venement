...
<?php
   ...
   *     declination_url: NULL
   *     declinations_name: string, "declinations"
   *     declinations:
   *       [declination_id]: integer
   *         name: xxx
   *         id: integer
   *         type: string, 'pdt-declination'
   *         url: NULL
   *         description: string, description
   *         available_prices:
   *           [id]: integer
   ...
   **/
    ...
    {
      // by manifestation
      if ( !isset($this->json[$pid = $item instanceof BoughtProduct ? $item->Declination->product_id : $item]) )
      {
      ...
      if (!( isset($this->json[$item->Declination->product_id]['declinations'][$item->product_declination_id]['prices'][$pname])
          && count($this->json[$item->Declination->product_id]['declinations'][$item->product_declination_id]['prices'][$pname]['ids']) > 0
      ))
        $this->json[$item->Declination->product_id]['declinations'][$item->product_declination_id]['prices'][$pname] = array(
          'state' => $state,
      ...
      $this->json[$item->Declination->product_id]['declinations'][$item->product_declination_id]['prices'][$pname]['ids'][] = $item->id;
      
      // by group of tickets
      $this->json[$item->Declination->product_id]['declinations'][$item->product_declination_id]['prices'][$pname]['qty']++;
      $this->json[$item->Declination->product_id]['declinations'][$item->product_declination_id]['prices'][$pname]['pit'] += $item->value;
      $this->json[$item->Declination->product_id]['declinations'][$item->product_declination_id]['prices'][$pname]['tep'] += $tep = round(($item->value)/(1+$item->vat),2);
      $this->json[$item->Declination->product_id]['declinations'][$item->product_declination_id]['prices'][$pname]['vat'] += $item->value - $tep;
    }
