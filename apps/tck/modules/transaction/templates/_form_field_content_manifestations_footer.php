...
<form action="<?php echo url_for('ticket/print?id='.$transaction->id) ?>"
  ...
  <p>
    ...
    <input type="checkbox" name="duplicate" value="true" title="<?php echo __('Duplicatas') ?>" onclick="javascript: if ( $('#li_transaction_manifestation .item.ui-state-highlight').length == 0 ) { $(this).prop('checked',false); LI.alert($(this).closest('form').find('.choose-a-manifestation').text()); return false; } $(this).hide(); $(this).closest('form').find('[name=price_name]').show().focus();" />
