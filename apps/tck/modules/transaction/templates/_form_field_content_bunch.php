...
<?php if (! (isset($detail['fake']) && $detail['fake']) ): ?>
  <script type="text/javascript">
    function liCompleteContent(data, id, replaceAll = true)
    {
      ...
      var wglobal = $('#li_transaction_'+id+' .families.sample').clone(true)
        .removeClass('sample');
      // currency
      var currency = $('#li_transaction_'+id+' .currency').html();
      
      if ( replaceAll )
      {
        $('#li_transaction_'+id+' .families:not(.sample)').remove();
        wglobal.find('.family:not(.total)').remove();
        wglobal.appendTo($('#li_transaction_'+id));
      }
      
      $.each(data, function(manifestation_id, manifestation){
        var wmanif = $('#li_transaction_'+id+' .families.sample .family:not(.total)').clone(true);
        var add = true;
        ...
        $.each(manifestation.gauges, function(index, gauge){
          var wgauge = $('#li_transaction_'+id+' .families.sample .item:not(.total)').clone(true);
          var add = true;
          ...
          $.each(gauge['prices'], function(index, price){
            var wprice = $('#li_transaction_'+id+' .families.sample .declination').clone(true);
            var add = true;
      ...
      $('#li_transaction_'+id+' .item .total').select();
    }
    
    $(document).ready(function(){
      $.get('<?php echo url_for($detail['data_url'].'?id='.$transaction->id) ?>',function(data){
        liCompleteContent(data, 'manifestations');
      });
    });
  </script>
