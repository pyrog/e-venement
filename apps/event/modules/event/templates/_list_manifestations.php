  ...
  <?php
    $manifs[$manif->happens_at.'-'.$manif->id] =
      link_to(format_date($manif->happens_at,'EEE d MMM yyyy HH:mm'),
        ...
        array('title' => format_date(strtotime($manif->happens_at) + strtotime($manif->duration) - strtotime('0:00'), 'EEE d MMM yyyy HH:mm').' @ '.$manif->Location, 'style' =>  $manif->Color ? 'background-color: #'.$manif->Color->color.';' : '')
