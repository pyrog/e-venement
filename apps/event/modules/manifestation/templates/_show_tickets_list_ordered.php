<h2><?php echo __('Ordered tickets') ?></h2>
<table class="ordered">
<tbody>
  ...
  <?php
    ...
    if ( !$t->printed && $t->Transaction->Order->count() > 0 )
    {
      ...
      if ( $t->Transaction->professional_id )
        $pro[$t->Transaction->professional_id] = cross_app_link_to($t->Transaction->Professional->Contact.' '.$t->Transaction->Professional,'rp','contact/show?id='.$t->Transaction->Contact->id);
