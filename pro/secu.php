...
<?php
	...
	// module inactif
	if ( !in_array("pro",$config["mods"]) )
	{
		$user->addAlert("Module inexistant");
		$nav->redirect($config["website"]["base"],"module désactivé");
	}
	
	if ( !isset($user->prolevel) )
	{
		$query	= " SELECT level FROM pro.rights WHERE id = ".$user->getId();
		$request = new bdRequest($bd,$query);
		$user->prolevel = intval($request->getRecord("level"));
		$request->free();
	}
	
	if ( $user->prolevel < $config["pro"]["right"]["view"] && !$user->hasRight($config["right"]["param"]) && !headers_sent() )
	{
