...
<?php
	...
	{
		$arr = array();
		$arr[$name = "name"] = trim($content[$name]) ? trim($content[$name]) : NULL;
		
		$msg = "L'espace ".$arr[$name]." n'a pu être modifié.";
		if ( $arr[$name] != "" )
		{
	...
	{
		$arr = array();
		$arr[$name = "name"] = trim($new[$name]) ? trim($new[$name]) : NULL;
...
<div class="body">
<h2>Les espaces de travail en billetterie</h2>
<form action="<?php $_SERVER["PHP_SELF"]; ?>" method="post" class="tarif"><ul><?php
	echo '<li class="th">';
	echo '<span class="del">suppr.</span>';
	echo '<span class="id"></span>';
	echo '<span class="desc">'."Nom de l'espace</span>";
	echo '</li>';
	
	echo '<li class="new">';
	echo '<span class="del"></span>';
	echo '<span class="id"></span>';
	echo '<span class="desc"><input type="text" id="focus" value="" name="new[name]" /></span>';
	echo '</li>';
	...
	while ( $rec = $request->getRecordNext() )
	{
		...
		echo '<span class="desc"><input type="text" value="'.htmlsecure($rec['name']).'" name="upd['.intval($rec["id"]).'][name]" /></span>';
		echo '</li>';
	}
?></ul>
