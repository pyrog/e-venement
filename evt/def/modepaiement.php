...
<?php
	...
	if ( !$bd->delRecordsSimple($table,array("id" => intval($value))) )
		$user->addAlert("L'enregistrement ".intval($value)." n'a pu être effacé.");
	
	// les MAJ
	if ( is_array($upd = $_POST["upd"]) )
	foreach ( $upd as $id => $content )
	if ( is_array($content) )
	{
		$arr = array();
		$arr[$name = "libelle"] = trim($content[$name]) ? trim($content[$name]) : NULL;
		$arr[$name = "numcompte"] = trim($content[$name]);
		
		$msg = "Le mode de paiement ".$arr["libelle"]." n'a pu être modifié.";
		if ( $arr["libelle"] != "" && $arr["numcompte"] )
		{
			if ( !$bd->updateRecordsSimple($table, array("id" => intval($id)), $arr) )
				$user->addAlert($msg);
		}
	}
	...
	if ( is_array($new = $_POST["new"]) )
	{
		...
		$arr[$name = "numcompte"] = trim($new[$name]) ? trim($new[$name]) : NULL;
...
<form action="<?php $_SERVER["PHP_SELF"]; ?>" method="post" class="tarif"><ul><?php
	...
	echo '<span class="cpt">Numéro de compte</span>';
	echo '</li>';
	
	echo '<li class="new">';
	echo '<span class="del"></span>';
	echo '<span class="id"></span>';
	echo '<span class="desc"><input type="text" id="focus" value="" name="new[libelle]" /></span>';
	echo '<span class="cpt"><input type="text" value="" name="new[numcompte]" maxlength="30" /></span>';
	echo '</li>';
	
	while ( $rec = $request->getRecordNext() )
	{
		echo '<li class="upd">';
		echo '<span class="del"><input type="checkbox" name="del[]" value="'.intval($rec["id"]).'" /></span>';
		echo '<span class="id">'.$rec["id"].'</span>';
		echo '<span class="desc"><input type="text" value="'.htmlsecure($rec["libelle"]).'" name="upd['.intval($rec["id"]).'][libelle]" /></span>';
		echo '<span class="cpt"><input type="text" value="'.htmlsecure($rec["numcompte"]).'" name="upd['.intval($rec["id"]).'][numcompte]" maxlength="30" /></span>';
		echo '</li>';
	}
?></ul>
